...
from pyspark.sql.functions import upper, lit, regexp_extract, col, concat_ws, count, isnan, when, window, coalesce, avg, round
...
def perform_data_clean(df1, df2):
    ...
    try:
        logger.info(f"perform_data_clean() is started for df_city")
        ...
        # 8 Impute TRX_CNT where it is null as avg of trx_cnt for that prescriber
